                ...
                <table className="min-w-full divide-y divide-gray-300">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        ...
                        <tr key={cita.id_cit} className="hover:bg-gray-50">
                            ...
                            <button onClick={() => setCitaSeleccionada(cita)} className="text-blue-600 hover:text-blue-900 font-bold">Ver</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-xl w-full p-6 relative animate-fade-in-up">
                    ...
                    <div className="border-t pt-4">
                        ...
                        <div className="grid grid-cols-3 gap-4 bg-green-50 p-3 rounded border border-green-200">
                            <div>
                                ...
                                <span className="text-lg font-bold text-gray-800">${Number(citaSeleccionada.precioAcordado_cit || 0).toFixed(2)}</span>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-gray-500 uppercase block">Estado</label>
                                <span className={`font-bold text-sm ${citaSeleccionada.pagado_cit ? 'text-green-600' : 'text-red-500'}`}>
                                    {citaSeleccionada.pagado_cit ? 'PAGADO' : 'NO PAGADO'}
                                </span>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-gray-500 uppercase block">Total Pagado</label>
                                <span className="text-lg font-bold text-green-700">
                                    ${Number(citaSeleccionada.cantidadPagada_cit || 0).toFixed(2)}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="mt-6 flex justify-end gap-2">
                    <button onClick={() => router.push(`/citas/${citaSeleccionada.id_cit}`)} className="bg-pie-green text-white px-4 py-2 rounded hover:bg-pie-dark">
                        Editar Información
                    </button>
                    <button onClick={() => setCitaSeleccionada(null)} className="bg-gray-200 text-gray-700 px-4 py-2 rounded hover:bg-gray-300">
                        Cerrar
                    </button>
                </div>
            </div>
